...
class Genes(Resource):
    ...
    def get(self, gene_id=None):
        ...
        if gene_id:
            LOG.info("Call for: GET /genes/{}".format(gene_id))
            result, status = search_database.search(term=gene_id, exact=True)
        else:
            LOG.info("Call for: GET /genes/")
            # get all
            # 'args' look only in query string
            self.reqparse.add_argument('species', type=str, required=False, location='args')
            d = self.reqparse.parse_args()
            LOG.debug(d)
            result, status = search_database.search(term=gene_id, exact=True)
        ...
        print(type(result.matches[0].__dict__))
        return {'gene': result.matches[0].dict()}

    def put(self, gene_id):
        LOG.error("Call for: PUT /genes/%s [NOT YET IMPLEMENTED]" % gene_id)
        content = {'please move along': 'nothing to see here'}
        return content, 501

    def delete(self, gene_id):
        LOG.error("Call for: DELETE /genes/%s [NOT YET IMPLEMENTED]" % gene_id)
        content = {'please move along': 'nothing to see here'}
        return content, 501

    def post(self):
        LOG.error("Call for: POST /genes")
        content = {'please move along': 'nothing to see here'}
        return content, 501
...
def run(settings=None):
    ...
    api.add_resource(Genes, '/genes/<gene_id>', '/genes/<gene_id>/', '/genes', '/genes/')
